class GenreMatcher {
    ...
    getRelevantTracks(input) {
        ...
        const filtered = this.genres.filter((genre) =>
            genre.title.toLowerCase().startsWith(input.slice(0, 3))
        );
     ...
     */
    numRelevantGenres() {
        if (this.relevantGenres === null) {
            return 0;
        } else {
            return this.relevantGenres.length;
        }
    }
